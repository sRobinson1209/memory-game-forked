...
def generate_melody(play_note = False, note_number = 0):
    ...
    while checked_input is False: #executes if input is incorrect
        print("Incorrect input!\nPlease try again!")
        play_sound(MIDI_file)
    
        checked_input = check_user_input()

        if checked_input == "S" or checked_input == "s": #MAKE SURE THESE WORK
            checked_input == True
        
        if checked_input == "Q" or checked_input == "q": #MAKE SURE THESE WORK
        
            return f"You quit the game!"

    if checked_input: #move onto next level if input is now correct
